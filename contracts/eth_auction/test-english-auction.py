...
with open(os.path.join(__location__, "EnglishAuction.sol"), "r") as file:
    auction_contract_file = file.read()

compiled_auction_sol = compile_standard(
    {
        "language": "Solidity",
        "sources": {"EnglishAuction.sol": {"content": auction_contract_file}},
        "settings": {
            "outputSelection": {
                "*": {
                    "*": [
...
## get bytecode
auction_bytecode = compiled_auction_sol["contracts"]["EnglishAuction.sol"]["EnglishAuction"]["evm"][
    "bytecode"
]["object"]

# ## get abi
auction_abi = json.loads(
    compiled_auction_sol["contracts"]["EnglishAuction.sol"]["EnglishAuction"]["metadata"]
)["output"]["abi"]
...
print(f"Start Deploying EnglishAuction Contract!")
# send the transaction
...
print(f"Done! Contract EnglishAuction deployed to {auction_address}")
...
add_request_nonce = w3.eth.get_transaction_count(node_address)
tx_hash = deployed_auction_contract.functions.create("asset 1").transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": add_request_nonce})
receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
print(f"Done! Gas use: {receipt.gasUsed}")

print("Create another auction... ")
add_request_nonce = w3.eth.get_transaction_count(node_address)
tx_hash = deployed_auction_contract.functions.create("asset 2").transact({"chainId": chain_id, "gasPrice": w3.eth.gas_price, "from": node_address, "nonce": add_request_nonce})
receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
print(f"Done! Gas use: {receipt.gasUsed}")
...
print("User 0 issues a bid for the first auction... ")
add_request_nonce = w3.eth.get_transaction_count(w3.eth.accounts[2])
